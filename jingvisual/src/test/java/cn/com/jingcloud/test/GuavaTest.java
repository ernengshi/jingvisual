...
public class GuavaTest {
    ...
    ConcurrentHashMap<String, AtomicLong> map3 = new ConcurrentHashMap<String, AtomicLong>(); //线程安全，但也要注意使用方式
...
//                        map3.put(key, 1);
//                    }
                    AtomicLong number = map3.get(key);
                    if (number == null) {
                        AtomicLong newNumber = new AtomicLong(0);
                        number = map3.putIfAbsent(key, newNumber);
                        if (number == null) {
                            number = newNumber;
                        }
                    }
