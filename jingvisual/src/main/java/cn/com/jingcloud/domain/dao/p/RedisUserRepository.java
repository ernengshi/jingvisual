...
public interface RedisUserRepository extends JpaRepository<RedisUser, Long> {
    ...
    @Cacheable(key = "#p0")
    RedisUser findByName(String name);
    
    /**
     * RedisUser 实体类对应的 @Entity和@Table(name = "redis_user")  要写正确不然不能mapped
     * @param name
     * @param age
     * @return 
     */
    @Query("from RedisUser u where u.name like %?1% and u.age>?2")
    RedisUser findByName(String name, Integer age);
